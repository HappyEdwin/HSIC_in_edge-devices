...
class EMA1(nn.Module):
    def __init__(self, channels, c2=None, factor=32):
        ...
        #self.agp = nn.AdaptiveAvgPool2d((1, 1)) ONNX-safe
        #self.pool_h = nn.AdaptiveAvgPool2d((None, 1)) ONNX-safe
        #self.pool_w = nn.AdaptiveAvgPool2d((1, None)) ONNX-safe
    ...
    def forward(self, x):
        ...
        x_h = group_x.mean(dim=3, keepdim=True) # ONNX-safe
        x_w = group_x.mean(dim=2, keepdim=True).permute(0, 1, 3, 2) # ONNX-safe
        ...
        ###
        x_hw = x2.mean(dim=(2, 3), keepdim=True) #ONNX-safe
        x1_agp = x1.mean(dim=(2, 3), keepdim=True) #ONNX-safe
        ###
        x11 = self.softmax((x1_agp + x_hw).reshape(b * self.groups, -1, 1).permute(0, 2, 1)) #ONNX-safe
        ###
